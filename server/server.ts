...
app.post('/api/auth/verify-recovery', (req: Request, res: Response) => {
...
app.post('/api/support/suggestion', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const logDir = path.join(__dirname, '../suggestions');
    if (!fs.existsSync(logDir)) {
      fs.mkdirSync(logDir, { recursive: true });
    }
    const logPath = path.join(logDir, `feedback-${Date.now()}.json`);
    fs.writeFileSync(logPath, JSON.stringify({ email, suggestion, date: new Date().toISOString() }, null, 2));
...
app.post('/api/support/ticket', async (req: Request, res: Response) => {
  ...
  try {
    const localLogDir = path.join(__dirname, '../support_tickets');
    if (!fs.existsSync(localLogDir)) {
      fs.mkdirSync(localLogDir, { recursive: true });
    }

    const localFilePath = path.join(localLogDir, `ticket-${ticketId}.json`);
    fs.writeFileSync(localFilePath, JSON.stringify(ticketPayload, null, 2));
...
app.post('/api/diagnostics/query', (req: Request, res: Response) => {
  ...
  const keywords = cleanQuery.split(/[^a-z0-9]/).filter((word: string) => word.length > 2 && !stopWords.includes(word));
  ...
  if (activeRepoPath && fs.existsSync(activeRepoPath) && keywords.length > 0) {
    try {
      ...
      if (matchedFile) {
        const fullContent = fs.readFileSync(matchedFile, 'utf-8');
        relativePath = path.relative(activeRepoPath, matchedFile);
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          const hasKeyword = keywords.some((k: string) => lineLower.includes(k));
          if (hasKeyword) {
            focusLine = i;
            break;
          }
  ...
  let recommendedRepos: any[] = [];
  if (Array.isArray(connectedRepos) && keywords.length > 0) {
    recommendedRepos = connectedRepos.filter((repo: any) => {
      const name = (repo.name || '').toLowerCase();
      const desc = (repo.description || '').toLowerCase();
      
      return keywords.some((keyword: string) => name.includes(keyword) || desc.includes(keyword));
    });
  }
...
app.get('/api/admin/stats', (req: Request, res: Response) => {
  ...
  try {
    ...
    const users = (userService as any).loadUsers(); // bypass TS private access rule
    ...
    const tickets: any[] = [];
    const ticketsDir = path.join(__dirname, '../support_tickets');
    if (fs.existsSync(ticketsDir)) {
      const files = fs.readdirSync(ticketsDir);
      files.forEach(file => {
        if (file.endsWith('.json')) {
          try {
            const data = fs.readFileSync(path.join(ticketsDir, file), 'utf-8');
    ...
    if (fs.existsSync(suggestionsDir)) {
      ...
      files.forEach(file => {
        if (file.endsWith('.json')) {
          try {
            const data = fs.readFileSync(path.join(suggestionsDir, file), 'utf-8');
    ...
    if (fs.existsSync(reposDir)) {
      scannedReposCount = fs.readdirSync(reposDir).length;
    }
...
app.post('/api/assistant/chat', async (req: Request, res: Response) => {
  ...
  try {
    if (isMissingFeature) {
      ...
      const logPath = path.join(logDir, `ai-copilot-missing-${Date.now()}.json`);
      fs.writeFileSync(logPath, JSON.stringify({ email: user.identifier, query: message, date: new Date().toISOString() }, null, 2));
